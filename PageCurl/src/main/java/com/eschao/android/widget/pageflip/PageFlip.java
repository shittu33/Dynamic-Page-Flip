...
public class PageFlip {
     ...
     * <p>Mask alpha will be invalid in double pages</p>
     *
     * @param alpha alpha value is in [0..1]
     * @return self
     */
    public PageFlip setMaskAlphaOfFold(float alpha) {
        mFoldBackVertexes.setMaskAlpha(alpha);
        return this;
    }
    /**
    ...
    public void setFlipState(PageFlipState flipState) {
        mFlipState = flipState;
    }
...
public boolean is_back_restore;
    /**
    ...
    public boolean onFingerUp(float touchX, float touchY, int duration) {
        ...
        // backward flipping
            ...
            if (!page.isXInRange(touchX, 0.8f)) {
                /**
                 * In case something happen change it back to FORWARD_FLIP
                 * */
//                mFlipState = PageFlipState.RESTORE_BACKWARD;
                is_back_restore = true;
//                mFlipState = PageFlipState.FORWARD_FLIP;
//                mFlipState = PageFlipState.RESTORE_FLIP;
                end.set((int)(diagonalP.x - page.width), (int)originP.y);
            }
            else {
                is_back_restore = false;
