...
public class DoublePagesRender extends PageRender {
     ...
     * Constructor
     *
     * @see {@link #DoublePagesRender(Context, PageFlip, Handler, int)}
     */
    ...
    private void drawPage(int number) {
        ...
        Bitmap background = LoadBitmapTask.get(mContext).getBitmap(number - 1);
        Rect rect = new Rect(0, 0, width, height);


        if (bitmap_matrix!=null){
            mCanvas.setMatrix(bitmap_matrix);
        }
//        if (scale > 0.2f) {
